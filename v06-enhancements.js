...
function indexAttachments(rows){attachments=rows||[];bySpecimen=new Map();bySource=new Map();for(const a of attachments){if(a.source_path)bySource.set(a.source_path,a);const key=a.specimen_code||'_collection';if(!bySpecimen.has(key))bySpecimen.set(key,[]);bySpecimen.get(key).push(a);}for(const list of bySpecimen.values())list.sort((a,b)=>String(a.created_at).localeCompare(String(b.created_at)));}
async function refreshAttachments(){if(!LISCloud.signedIn())return;indexAttachments(await LISCloud.listAttachments(null));queuePatch();}
...
function photoRows(code){return (bySpecimen.get(code)||[]).filter(a=>a.material_type==='Specimen photograph'||String(a.mime_type||'').startsWith('image/'));}
