...
const portrait=$('#portraitUpload');if(portrait)portrait.closest('.portrait-control')?.classList.toggle('hidden',bySource.has('images/about/jane-freese.jpg'));const guide=$('#guideUpload');if(guide)guide.closest('.guide-control')?.classList.toggle('hidden',bySource.has('images/about/jane-mineral-guide.jpg'));
}
...
async function backup(){const records=LISApp.getRecords(),inventory=attachments.map(({owner_id,...a})=>a),payload={version:'0.6.7',createdAt:new Date().toISOString(),catalogRecords:records,privateAttachmentInventory:inventory,note:'Private file contents remain in Supabase Storage. This backup preserves the catalog and attachment index.'};download(`Legacy_in_Stone_Cloud_Backup_${new Date().toISOString().slice(0,10)}.json`,new Blob([JSON.stringify(payload,null,2)],{type:'application/json'}));toast('Cloud catalog and attachment inventory exported.','success');}
async function migratePetrifiedWood(){
  if(!LISCloud.signedIn()){toast('Sign in before moving specimens.','error');return;}
  const records=LISApp.getRecords();
  const candidates=records.filter(x=>x.collection_type!=='Fossils'&&/petrified\s+wood/i.test([x.name,x.identification].filter(Boolean).join(' '))).sort((a,b)=>String(a.specimen_code||'').localeCompare(String(b.specimen_code||''),undefined,{numeric:true}));
  if(!candidates.length){toast('No non-fossil petrified wood specimens were found.','success');return;}
  const fossilNums=records.map(x=>String(x.specimen_code||'').match(/^F-(\d+)$/i)).filter(Boolean).map(m=>Number(m[1])).filter(Number.isFinite);
  let next=(fossilNums.length?Math.max(...fossilNums):0)+1;
  const assignments=candidates.map(record=>({record,newCode:`F-${String(next++).padStart(3,'0')}`}));
  const summary=assignments.map(x=>`${x.record.specimen_code} → ${x.newCode}`).join('\n');
  if(!confirm(`Move ${assignments.length} petrified wood specimen${assignments.length===1?'':'s'} to Fossils and renumber them?\n\n${summary}\n\nExisting private photos and documents will stay linked to the specimens.`))return;
  const btn=$('#migratePetrifiedWood');setBusy(btn,true,'Moving specimens…');
  try{
    for(const {record,newCode} of assignments){
      const legacy=Number((newCode.match(/\d+/)||['0'])[0]);
      const updated={...record,specimen_code:newCode,legacy_number:legacy,collection_type:'Fossils',collection_type_id:3};
      await LISCloud.renameSpecimen(record.specimen_code,updated);
    }
    const fresh=await LISCloud.pullCatalog();
    LISApp.replaceRecords(fresh);
    await refreshAttachments();
    toast(`${assignments.length} petrified wood specimen${assignments.length===1?'':'s'} moved to Fossils and renumbered.`,'success');
  }catch(err){
    toast('Petrified wood move failed: '+err.message,'error');
    try{await loadCloudCatalog();}catch(_){}
...
async function uploadGuide(file){if(!file?.name)return;try{setMessage('Uploading Jane Freese mineral guide photograph…','info');await LISCloud.uploadAttachment({file,specimenCode:null,type:'Collection history photograph',caption:'Jane Freese’s Mineral Guide',sourcePath:'images/about/jane-mineral-guide.jpg'});await refreshAttachments();setMessage('Mineral guide photograph saved in private storage.','success');}catch(err){setMessage('Mineral guide upload failed: '+err.message,'error');}}
function setupCloud(){refreshStatus();$('#cloudLoginForm').onsubmit=e=>{e.preventDefault();authenticate(e.target.email.value,e.target.password.value,e.target)};$('#authLoginForm').onsubmit=e=>{e.preventDefault();$('#authError').classList.add('hidden');authenticate(e.target.email.value,e.target.password.value,e.target)};$('#cloudSignOut').onclick=()=>{LISCloud.signOut();indexAttachments([]);refreshStatus();lock();setMessage('Signed out.','success')};$('#cloudPush').onclick=async()=>{const btn=$('#cloudPush');setBusy(btn,true,'Saving all…');clearMessage();try{const records=LISApp.getRecords();await LISCloud.pushCatalog(records);const now=new Date().toISOString();localStorage.setItem('lis-last-sync',now);refreshStatus();setMessage(`${records.length} records saved to the private cloud.`,'success');}catch(err){setMessage('Full catalog save failed: '+err.message,'error');}finally{setBusy(btn,false)}};const portrait=$('#portraitUpload');if(portrait)portrait.onchange=()=>uploadPortrait(portrait.files?.[0]);const guide=$('#guideUpload');if(guide)guide.onchange=()=>uploadGuide(guide.files?.[0]);if(!LISCloud.configured()){lock();setMessage('Cloud connection is not configured.','error');}else if(LISCloud.signedIn()){unlock();loadCloudCatalog();}else lock();}
window.addEventListener('DOMContentLoaded',()=>{const settings=$('#settingsDialog .settings-actions');if(settings&&!$('#portraitUpload'))settings.insertAdjacentHTML('beforeend','<label class="outline portrait-control">Add Jane Freese portrait<input id="portraitUpload" type="file" accept="image/*" hidden></label>');if(settings&&!$('#guideUpload'))settings.insertAdjacentHTML('beforeend','<label class="outline guide-control">Add mineral guide photograph<input id="guideUpload" type="file" accept="image/*" hidden></label>');$('#attachmentForm').onsubmit=saveAttachment;$('#fullBackup').onclick=backup;const migrate=$('#migratePetrifiedWood');if(migrate)migrate.onclick=migratePetrifiedWood;$('#settingsButton').onclick=()=>{$('#settingsDialog').showModal();refreshStatus()};new MutationObserver(queuePatch).observe(document.body,{childList:true,subtree:true});setupCloud();if('serviceWorker'in navigator&&location.protocol.startsWith('http'))navigator.serviceWorker.register('./service-worker.js');const install=$('#installApp');let deferred;window.addEventListener('beforeinstallprompt',e=>{e.preventDefault();deferred=e;install.hidden=false});install.onclick=async()=>{if(deferred){deferred.prompt();await deferred.userChoice;deferred=null;install.hidden=true}else toast('Use your browser’s Add to Home Screen or Install App command.')};});
})();
