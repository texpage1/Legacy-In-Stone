...
let toastTimer=null;
function toast(msg,kind='info'){const t=$('#toast');t.textContent=msg;t.dataset.kind=kind;t.classList.remove('hidden');clearTimeout(toastTimer);toastTimer=setTimeout(()=>t.classList.add('hidden'),6000);}
function setMessage(msg,kind='info'){const m=$('#cloudMessage');m.textContent=msg;m.dataset.kind=kind;m.classList.remove('hidden');}
function clearMessage(){const m=$('#cloudMessage');m.textContent='';m.classList.add('hidden');}
...
if(LISCloud.signedIn()){const specimen=specimenById(id);try{const cloud=await LISCloud.listAttachments(specimen?.specimen_code);if(cloud.length){const wrap=document.createElement('div');wrap.className='materials-list cloud-materials';wrap.innerHTML=cloud.map(a=>`<button class="material-item local-attachment"><span class="material-type">Private cloud</span><strong>${esc(a.file_name)}</strong><small>${esc(a.caption||a.material_type||'Open private file')}</small></button>`).join('');host.appendChild(wrap);wrap.querySelectorAll('button').forEach((b,i)=>b.onclick=async()=>{try{window.open(await LISCloud.signedUrl(cloud[i].storage_path),'_blank','noopener')}catch(err){toast(err.message,'error')}});}}catch(_){} }}
function observeDetail(){const body=$('#detailBody');new MutationObserver(()=>{const code=body.querySelector('.code')?.textContent?.trim();if(!code)return;const specimen=(LISApp?.getRecords?.()||[]).find(x=>x.specimen_code===code);if(specimen)renderLocalMaterials(specimen.id);}).observe(body,{childList:true,subtree:true});}
function statusText(){if(!LISCloud.configured())return'Cloud account not connected';return LISCloud.signedIn()?'Signed in and ready to sync':'Cloud configured; sign in required';}
function refreshStatus(){const s=$('#cloudStatus');s.textContent=statusText();const last=localStorage.getItem('lis-last-sync');$('#lastSync').textContent=last?`Last synchronized: ${new Date(last).toLocaleString()}`:'Catalog has not been synchronized from this device.';$('#cloudSignOut').disabled=!LISCloud.signedIn();$('#cloudPush').disabled=!LISCloud.signedIn();$('#mediaMigration').disabled=!LISCloud.signedIn();}
async function loadCloudCatalog(){if(!LISCloud.signedIn())return;try{const records=await LISCloud.pullCatalog();if(records.length){LISApp.replaceRecords(records);setMessage(`${records.length} cloud records loaded.`,`success`);}}catch(err){setMessage('Could not load the cloud catalog: '+err.message,'error');}}
